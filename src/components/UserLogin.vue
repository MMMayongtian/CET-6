    ...
    <div name="el-zoom-in-top">
      <div>
        <el-form ref="loginFormRef" :model="loginForm" :rules="loginFormRules" class="sign-up">
          ...
          <!-- 用户名区域 -->
          <input v-model="loginForm.username" type="text" class="sign-up-input" placeholder="What's your username?"
            autofocus>
          <!-- 密码区域 -->
...
import { addUserRequest } from "network/user";
// 验证邮箱的规则
  ...
  methods: {
    ...
    addUser() {
      this.$refs.addUserFormRef.validate(valid => {
        if (!valid) return;
        loginRequest({ username: "register", password: "000000" }).then(res => {
          let result = res.data;
