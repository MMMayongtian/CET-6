...
| goods_state  | 试题状态     | 试题状态 0: 未通过 1: 审核中 2: 已审核 |
| add_time     | 添加时间     |                                        |
| upd_time     | 更新时间     |                                        |
| hot_mumber   | 热销品数量   |                                        |
| is_promote   | 是否是热销品 |                                        |
...
| attrs           | 试题的参数（数组），包含 `动态参数` 和 `静态属性` | 可以为空 |
...
| hot_mumber   | 热销品数量                 |                                                              |
| is_promote   | 是否是热销品               |                                                              |
| pics         | 上传的图片临时路径（对象） | pics_id:图片 ID,goods_id:试题 ID,pics_big:大图,pics_mid:中图,pics_sma:小图 |
| attrs        | 试题的参数（数组）         | goods_id:试题 ID,attr_value:当前试题的参数值,add_price:浮动价格,attr_vals:预定义的参数值,attr_sel:手动输入，还是单选, |
...
| ------ | ------------ | ------------------------------------------------------------ |
| id     | 试题 ID      | 不能为空`携带在url中`                                        |
| pics   | 试题图片集合 | 如果有 pics_id 字段会保留该图片，如果没有 pics_id 但是有 pic 字段就会新生成图片数据 |
...
| 参数名       | 参数说明     | 备注                                       |
| ------------ | ------------ | ------------------------------------------ |
| id           | 缴费 ID      | 不能为空`携带在url中`                      |
| is_send      | 缴费参加考试 | 1:已经发货，0:未发货                       |
| order_pay    | 缴费支付     | 支付方式 0 未支付 1 支付宝 2 微信 3 银行卡 |
| order_price  | 缴费金额     |                                            |
| order_number | 缴费数量     |                                            |
| pay_status   | 支付状态     | 缴费状态： 0 未付款、1 已付款              |
  ...
  ```json
  {
    "data": [
      ...
      },
      {
        ...
        "context": "您的缴费将由HLA（北京海淀区清河中街店）门店安排发货。",
        "location": ""
      },
      {
